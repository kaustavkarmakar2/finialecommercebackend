...
var token;
const s3ProfileUpload = require('./../../store/s3Upload');
...
module.exports.controllerFunction = function(app,connection){
    ...
    userRouter.post('/catalog', function(req,res){
        var product_id = req.body.product_desc;
        ...
        connection.query(`SELECT * FROM Product_Catalog  `, function (err, result, fields) {
            if (err) {
                throw err;
                console.log('========================',err);
    ...
    userRouter.post('/uploadProductImage',function (req, res, next){
        ...
        new Promise((resolve, reject)=>{
            upload(req, res, (err) => {
                ...
                const bucketName = "product_imageUrl_test"
                let resposePath = "https://" + bucketName + ".s3-us-west-1.amazonaws.com/" + product_id + '/' + filename;

                connection.query(`Update Product_Catalog set product_imageUrl = '" + resposePath   + "' where product_id = ' `, function (err, result, fields) {
                if (err) {
                        throw err;
                        console.log('========================',err);
                        res.send({success:false,message:'null'})
                ...
                let success;
                try{
                   success = await s3ProfileUpload.upload(bucketName , filename, path.resolve("./ProductPictureUploads") + '/', product_id)
